...
class YamlEditorApp:
    ...
    def setup_ui(self):
        ...
        # YAML block replacer button
        ttk.Button(main_frame, text="YAML Block Replacer", command=self.open_vehicle_replacer).grid(row=9, column=0, columnspan=2, pady=5)
    ...
    def on_property_selected(self, event):
        ...
        # Check if the selected property has nested structure
        if selected_prop == "Item":
            ...
            self.item_sub_property_combo['values'] = item_properties
        elif (selected_prop in first_yaml['TestCase'][selected_main][selected_sub] and 
              isinstance(first_yaml['TestCase'][selected_main][selected_sub][selected_prop], dict)):
            # If the property is a dictionary (has nested structure), enable sub-property selector
            self.item_sub_property_combo['state'] = 'readonly'
            nested_properties = list(first_yaml['TestCase'][selected_main][selected_sub][selected_prop].keys())
            self.item_sub_property_combo['values'] = nested_properties
        else:
            ...
            if selected_item_prop:
                ...
                for i in range(sub_start_line, len(lines)):
                    ...
                    if i > sub_start_line + 20:  # prevent excessive search
                        break
        elif self.item_sub_property_combo['state'] == 'readonly' and self.item_sub_property_var.get():
            # Handle nested properties (like SeatPosition with BackrestTilt, Position)
            selected_nested_prop = self.item_sub_property_var.get()
            current_indent = None
            for i in range(sub_start_line, len(lines)):
                line = lines[i]
                if not current_indent and line.strip():
                ...
                if selected_prop in line:
                    ...
                    if indent == current_indent + 2:  # make sure it's a property under the sub-category
                        ...
                        if prop_in_line == selected_prop:
                            # Find the nested property within this property
                            for j in range(i + 1, len(lines)):
                                ...
                                # Check if we've moved to the next property at the same level
                                if nested_indent <= indent:
                                    break
                                    
                                # Check if this is the nested property we're looking for
                                if selected_nested_prop in nested_line and nested_indent > indent:
                                    nested_prop_in_line = nested_line.split(":")[0].strip()
                                    if nested_prop_in_line == selected_nested_prop:
                                        # Only show the value of the specific nested property
                                        value_lines = []
                                        
                                        # get the value of the first line (after the colon)
                                        first_line = nested_line.split(":", 1)[1]
                                        value_lines.append(first_line.rstrip('\n'))
                                        
                                        # check the following lines for multi-line values
                                        next_line_idx = j + 1
                                        while next_line_idx < len(lines):
                                            next_line = lines[next_line_idx]
                                            next_indent = len(next_line) - len(next_line.lstrip())
                                            
                                            # if the indent is less than or equal to the indent of the current nested property, stop
                                            if next_indent <= nested_indent:
                                                break
                                            
                                            # add the line if it's part of this nested property's value
                                            value_lines.append(next_line.rstrip('\n'))
                                            next_line_idx += 1
                                ...
                                if j > i + 20:  # prevent excessive search
                                    break
                            break
                    if i > sub_start_line + 20:  # prevent excessive search
                        break
